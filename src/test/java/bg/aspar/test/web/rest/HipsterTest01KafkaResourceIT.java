...
import bg.aspar.test.config.KafkaProperties;
...
class HipsterTest01KafkaResourceIT {
    ...
    void setup() {
        ...
        HipsterTest01KafkaResource kafkaResource = new HipsterTest01KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hipster-test-01-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/hipster-test-01-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
